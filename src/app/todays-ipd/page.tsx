...
const TodaysIpdDashboard = () => {
  ...
  const handleVisitIdClick = (patientId: string, visitId: string) => {
    ...
    router.push(`/patient-profile?patient=${patientId}&visit=${visitId}`);
  };
  ...
  const handleBillClick = async (visit) => {
    ...
    if (!isESICPatient) {
      router.push(`/final-bill/${visit.visit_id}`);
      return;
    }
                ...
                <TableRow key={visit.id} className="hover:bg-muted/50">
                  ...
                  <TableCell>
                    <Button
                      ...
                      onClick={() => router.push(`/admission-notes/${visit.visit_id}`)}
                  ...
                  <TableCell>
                    <Select onValueChange={(value) => {
                      if (value === 'discharge-summary') {
                        router.push(`/ipd-discharge-summary/${visit.visit_id}`);
                      } else if (value === 'death-certificate') {
                        router.push(`/death-certificate/${visit.visit_id}`);
                      }
                    }}>
